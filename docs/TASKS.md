...
- **Next task:** `M0-GATE` — human review of the M0 foundation; push `m0-foundation` and open a pull request against `main` to observe the first CI run (`M0-006`; the workflow triggers on pull requests and on pushes to `main`, not on other branches).
- **Current milestone:** M0 — repository foundation.
- **Active blockers:** `M0-006`/`M0-GATE`: the branch is intentionally unpushed and has no pull request yet, so the hosted CI run and reviewer sign-off are pending.
...
| M0-006 | `IN PROGRESS` | Add CI for Python lint/type/test and C++ configure/build/test | M0-003, M0-004, M0-005 | `.github/workflows/ci.yml`: jobs `python` (3.12/3.13 matrix: selective submodule init, headless Qt libs, `uv sync --locked`, `nox -s lint type_check tests`, coverage artifact), `cpp` (configure with `-Werror`, build, ctest), `pre-commit` (dev group only); uv pinned to 0.12.5, no secrets, `permissions: contents: read`. YAML validated and every job command executed locally from a clean clone; the acceptance "all jobs pass" awaits the first GitHub Actions run after the branch is pushed; review round 1: the Python job sets `ARM_RC_CTRL_EXPECTED_PYTHON` from the matrix so a fallback to 3.12 fails the job; verified locally that `UV_PYTHON=3.13` runs the whole gate under 3.13 |
...
| M0-GATE | `IN PROGRESS` | Review and close the M0 gate | M0-006, M0-007, M0-008, M0-009, M0-012, M0-013, M0-014 | Automated evidence (2026-08-29, fresh clone @ `9808640`): documented bootstrap reproduced every pin; `uv sync`; `uv run nox` = lint + basedpyright strict + 172 tests passed (1 strict xfail, UP-005) + CTest 2/2 with `-Werror`; two non-exploratory smoke runs on the clean tree produced identical canonical digest `25517ac9b02306ff…` with `project_dirty=false`; all pre-commit hooks passed; all submodules clean afterwards; no machine paths in tracked files. Remaining for closure: human reviewer reproduction/sign-off and the first hosted CI run (`M0-006`) after the branch is pushed |
