...
| M1-024 | `DONE` | Define an equal-budget baseline-gain tuning protocol | M1-022, M1-023 | `configs/studies/baseline_gains_1a.toml` fixes the scenario, budget (64 trials per tracker), sampler seed, objective (median movement-window joint RMSE over development scenarios) with infeasibility penalty, per-tracker log-uniform `kp`/`kd` search spaces, and development initial-posture offsets (distinct from confirmatory seeds); `arm_rc_ctrl.experiments.tuning` provides the typed `TuningProtocol`, seeded per-joint sampling, `evaluate_gains` (all objective components kept per scenario; early termination/limit violation/missed dwell → penalty), and deterministic `run_study` returning every trial and the selected one; `tests/integration/test_tuning.py` (11 cases). Studies themselves run in M1-025/M1-026 on the real dataset |
