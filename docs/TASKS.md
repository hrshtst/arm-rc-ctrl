...
- **Next task:** `M0-GATE` — clean-room walkthrough and gate review.
...
| M0-008 | `IN PROGRESS` | Add README setup, quality, and recursive-checkout commands | M0-003, M0-005 | `README.md` documents requirements, clean-checkout setup (selective submodule bootstrap, `uv sync`), the nox quality gate and raw commands, pin-advance reinstall, the external storage root, the smoke experiment, layout, licensing/citation; `AGENTS.md` commands aligned; `pyproject.toml` references the README. Clean-room walkthrough pending (recorded with the M0 gate evidence) |
