...
- **Next task:** `M0-GATE` — human review of the M0 foundation; push `m0-foundation` to observe the first CI run (`M0-006`).
- **Current milestone:** M0 — repository foundation.
- **Active blockers:** `M0-006`/`M0-GATE`: the branch is intentionally unpushed, so the hosted CI run and reviewer sign-off are pending.
